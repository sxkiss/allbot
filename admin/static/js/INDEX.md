<!-- AUTO-DOC: Update me when files in this folder change -->

# js

管理后台前端脚本：负责页面数据加载、交互行为和 API 调用。

## Files

| File | Role | Function |
|------|------|----------|
| config_form.js | Shared UI | 通用可视化配置表单（schema 渲染、列表/多机器人卡片、object_list/object_map、visual/raw 切换） |
| plugin_market.js | Market UI | 插件市场列表渲染、分页（每页10条）、搜索过滤、安装与提交 |
| plugins.js | Plugin UI | 插件管理页面逻辑（启停、可视化配置、市场联动） |
| webchat_widget.js | Widget | 全局 Web 对话悬浮组件 |
| feedback_widget.js | Widget | 全局意见反馈悬浮面板（提交到 /api/feedback） |
| adapters.js | Adapter UI | 适配器管理页面逻辑（可视化配置弹窗） |
| file-manager.js | File UI | 文件管理页面逻辑 |
| admin.js | Shared UI | 通用后台前端逻辑 |
| custom.js | Shared UI | 个性化交互与页面增强 |
| github_proxy.js | Tool UI | GitHub 代理配置页面逻辑 |
| fix_plugins_display.js | Patch | 插件显示修复脚本 |
| fix_dow_plugins.js | Patch | DOW 插件显示与行为修复脚本 |
